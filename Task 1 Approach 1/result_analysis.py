...
resultFile = 'Camelyon16/Testing/Testing Results/Model_InceptionV3/record.csv'
...
for threshold in thresholdList:
    ...
    resultThreshold['prediction_tumor'].values[resultThreshold['prediction_tumor'].values >= threshold] = 1
    resultThreshold['prediction_tumor'].values[resultThreshold['prediction_tumor'].values < threshold] = 0
    print(resultThreshold['prediction_tumor'])
    
    pred = tuple(resultThreshold['prediction_tumor'])
    groundTruth = tuple(resultThreshold['ground_truth'])
...
plt.legend([f'ResNet50 (AUC = {str(round(auc(allSen,allSpe),4))})','Random Classifier (AUC = 0.5000)'])
plt.show()
